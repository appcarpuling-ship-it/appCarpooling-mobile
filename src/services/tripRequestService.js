 ...
 */
/**
 * @param {object} [oferta] `{ driverPrice, sinPrecioFijo, requiereSena }`. driverPrice es
 ...
 *   Las otras dos son las mismas opciones que al publicar un viaje normal (TripDetails.js):
 *   `sinPrecioFijo` fuerza driverPrice a 0 del lado del server, y `requiereSena` pide la
 *   mitad por adelantado — el server la apaga si no hay precio (ver utils/sena.js).
 */
export const applyToTripRequest = (requestId, vehicleId, recorrido = {}, oferta = {}) =>
  post_withauth(ENDPOINTS.APPLY_TO_TRIP_REQUEST(requestId), {
    ...
    requiereSena: oferta.requiereSena === true,
    ...recorrido
  });
